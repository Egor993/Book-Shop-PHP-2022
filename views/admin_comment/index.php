                    ...
                    <?php foreach($comments as $comment): ?>
                        <tr class="row1">
                            <td class="action-checkbox">
                                <input type="checkbox" name="_selected_action[]" value="<?php echo $comment['id'];?>"
                                    class="action-select">
                            </td>
                            <th class="field-__str__">
                                <a href="#">
                                    ID <?php echo $comment['id'];?> Пользователь <?php echo $comment['name']?> отсавил
                                    комментарий: "<?php echo $comment['comment']?>" к книге
                                    "<?php echo Product::getProductById($comment['book_id'])['name'];?>"
                                </a>
                            </th>
                        </tr>
                    <?php endforeach;?>
